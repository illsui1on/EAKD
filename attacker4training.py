...
class AdversarialTrainingAttacker(object):
    ...
    def attack_all(self, n_candidate=100, n_iter=20, adv_sample_size=5000):
        ...
        for i in range(self.d.train.get_size()):
            ...
            tag, typ = self.attack(b['raw'][0], b['y'][0], self.syms['tr'][b['id'][0]], n_candidate, n_iter)
            if tag==True:
                n_succ += 1
...
if __name__ == "__main__":
    ...
    atk = Attacker(cc, symtab, classifier)  #变量名替换
    atk.attack_all(5, 40)

    #atk = InsAttacker(cc, instab, classifier) #死代码插入
    #atk.attack_all(5, 40)

    #atk = AttackerRandom(cc, symtab, classifier)  #baseline  变量名替换
    #atk.attack_all(40)

    #atk = InsAttackerRandom(cc, instab, classifier)   #baseline  死代码插入
